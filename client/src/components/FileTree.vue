...
function handleExport(data) {
  const message = data.destination === 'github'
    ? `Pushed ${data.count} files to ${data.repo}`
    : `Exported ${data.count} files to ${data.name}.zip`
...
function handleImport(data) {
  const message = data.source === 'github'
    ? `Imported ${data.count} files from ${data.repo}`
    : `Imported ${data.count} files (${data.mode} mode)`
  uiStore.showNotification(message, 'success')
